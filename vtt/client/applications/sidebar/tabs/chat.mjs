...
export default class ChatLog extends HandlebarsApplicationMixin(AbstractSidebarTab) {
  ...
  async #renderNotifications() {
    ...
    const html = await foundry.applications.handlebars.renderTemplate("templates/sidebar/tabs/chat/notifications.hbs", {
      rollModes
    });
    [this.#notificationsElement, this.#inputElement, this.#privacyButtons] = foundry.utils.parseHTML(html);
    ...
    this.#notificationsElement.append(this.#inputElement, this.#privacyButtons);
    this._toggleNotifications();
  ...
  async #postOne(message, {before, notify=false}={}) {
    ...
    if ( !before ) {
      let newerMessage;
      for ( let i = log.children.length; i--; ) {
        const msg = game.messages.get(log.children[i].dataset.messageId);
        if ( !msg ) continue;
  ...
  _updateRollMode() {
    ...
    const rollMode = game.settings.get("core", "rollMode");
    for ( const button of this.#privacyButtons.querySelectorAll("button") ) {
  ...
  #offsetHotbar(notifications) {
    ...
    const offset = (window.innerWidth / uiScale) - bb.right - rightWidth;
